...
class HubbleSeeOnBirthday(QWidget):
    ...
    def __init__(self, parent=None, title='你生日那天的宇宙', **kwargs):
        ...
        self.showLabelImage(PIL.Image.open(os.path.join(rootdir, 'resources/icon/icon.jpg')))
        # --显示介绍文字的text
            ...
            if url:
                ...
                while True:
                    try:
                        ...
                        continue
                html_root = lxml.etree.HTML(response.text)
                ...
                data = {'date': key, 'intro': intro, 'image': PIL.Image.open(filename), 'ext': image_url.split('.')[-1]}
            else:
                data = {'date': key, 'intro': ['地球上还不存在%s这个日期哦~' % key], 'image': PIL.Image.open(os.path.join(self.rootdir, 'resources/icon/icon.jpg')), 'ext': 'jpg'}
    ...
    def showLabelImage(self, image):
        image = image.resize((400, 300), PIL.Image.ANTIALIAS)
        fp = io.BytesIO()
        image.save(fp, 'BMP')
        qtimg = QtGui.QImage()
